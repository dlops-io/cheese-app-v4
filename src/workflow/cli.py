...
STACK_PATH = ".pulumi/stacks/deploy-images-ml/dev.json"


def get_pulumi_stack_outputs(
    bucket_name: str, stack_path: str, output_keys: list[str] | None = None
) -> dict:
    ...
    outputs = {}
    for resource in (
        stack_data.get("checkpoint", {}).get("latest", {}).get("resources", [])
    ):
...
def data_collector():
    ...
    image_tags = get_pulumi_stack_outputs(
        PULUMI_BUCKET, STACK_PATH, ["cheese-app-data-collector-tags"]
    )
    print(image_tags)
    ...
    def data_collector():
        container_spec = dsl.ContainerSpec(
            image=image_tags["cheese-app-data-collector-tags"][0],
            command=[],
            args=[
                "cli.py",
                "--search",
...
def data_processor():
    ...
    image_tags = get_pulumi_stack_outputs(
        PULUMI_BUCKET, STACK_PATH, ["cheese-app-data-processor-tags"]
    )
    print(image_tags)
    ...
    def data_processor():
        container_spec = dsl.ContainerSpec(
            image=image_tags["cheese-app-data-processor-tags"][0],
            command=[],
            args=[
                "cli.py",
                "--clean",
...
def pipeline():
    ...
    image_tags = get_pulumi_stack_outputs(
        PULUMI_BUCKET,
        STACK_PATH,
        ["cheese-app-data-collector-tags", "cheese-app-data-processor-tags"],
    )
    print(image_tags)
